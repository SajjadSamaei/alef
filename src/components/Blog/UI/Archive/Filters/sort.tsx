...
function Sort() {
  const { setSortConfig, sortConfig } = useBlogFilterContext();
        ...
        <Listbox value={sortConfig.key} onChange={handleKeyChange}>
          {({ open }) => (
            <>
              <ListboxButton
                className={clsx(
                  "group relative w-full cursor-pointer rounded-[40px] border bg-white/50 px-3 py-2 text-sm font-medium whitespace-nowrap text-neutral-950 shadow-xs transition-[color,box-shadow] outline-none lg:w-fit",
