...
const shapes = [
  ...
  {
    // Shape entry for a Squircle Approximation
    width: 100, // Base width for viewBox calculation
    height: 100, // Base height for viewBox calculation
    // Path data for a squircle approximated with cubic Beziers (n≈4)
    // Scaled to a 100x100 viewBox
    // Control points moved further out (90/10 vs 85/15) making curves tighter/corners sharper
    path: "M 50,0 C 90,0 100,10 100,50 C 100,90 90,100 50,100 C 10,100 0,90 0,50 C 0,10 10,0 50,0 Z",
  },
  {
    // Very Square Squircle (NEW - almost sharp corners)
    width: 100,
    height: 100,
    // Control points pushed extremely close to corners (98/2)
    path: "M 50,0 C 98,0 100,2 100,50 C 100,98 98,100 50,100 C 2,100 0,98 0,50 C 0,2 2,0 50,0 Z",
  },
  {
    // Near-Perfect Square Squircle (NEW - Minimal Curvature)
    width: 100,
    height: 100,
    // Control points pushed extremely close to corners (99/1) - almost sharp
    path: "M 50,0 C 99,0 100,1 100,50 C 100,99 99,100 50,100 C 1,100 0,99 0,50 C 0,1 1,0 50,0 Z",
  },
];
...
export function NormalImage({
  grayscale = true,
  shape = 0,
  className,
  ...props
}: ImagePropsWithOptionalAlt & Props) {
  let id = useId();
  let { width, height, path } = shapes[shape];

  return (
    <div className={clsx(className, "relative flex w-full")}>
      <Image
        alt=""
        className={clsx(
          "w-full overflow-hidden bg-neutral-100 object-cover",
          grayscale && "grayscale",
        )}
        style={{ aspectRatio: `${width} / ${height}` }}
        {...props}
