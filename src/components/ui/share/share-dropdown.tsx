  ...
  return (
    <Headless.MenuItems
      ...
      className={clsx(
        ...
        "flex grid-cols-[auto_1fr_1.5rem_0.5rem_auto] flex-col supports-[grid-template-columns:subgrid]:grid-cols-subgrid",
...
export function DropdownItem({
  ...
  let classes = clsx(
    className,
    // Base styles
    "group cursor-default rounded-2xl px-3.5 py-2.5 focus:outline-hidden sm:px-3 sm:py-1.5",
    // Text styles
    "text-right text-base/6  text-zinc-950 sm:text-sm/6 dark:text-white forced-colors:text-[CanvasText]",
    // Focus
    "data-focus:bg-neutral-700 data-focus:text-white",
    // Disabled state
    "data-disabled:opacity-50",
    // Forced colors mode
    "forced-color-adjust-none forced-colors:data-focus:bg-[Highlight] forced-colors:data-focus:text-[HighlightText] forced-colors:data-focus:*:data-[slot=icon]:text-[HighlightText]",
    // Use subgrid when available but fallback to an explicit grid layout if not
    "grid grid-cols-[auto_1fr_1.5rem_0.5rem_auto]  gap-2 supports-[grid-template-columns:subgrid]:grid-cols-subgrid",
    // Icons
    "*:data-[slot=icon]:col-start-1 *:data-[slot=icon]:row-start-1 *:data-[slot=icon]:ml-2.5 *:data-[slot=icon]:-mr-0.5 *:data-[slot=icon]:size-5 sm:*:data-[slot=icon]:mr-2 sm:*:data-[slot=icon]:size-4",
