...
export const getYearFromTimestamp = (date: Date): string => {
  const timestamp = new Date(date);
  const year = timestamp.getFullYear().toString();
  return year;
};

export const getMonthFromTimestamp = (date: Date): string => {
  const timestamp = new Date(date);
...
export const getPersianYearFromGregorian = (date: Date): string => {
  // Use moment-jalaali to ensure consistency with your query logic
  const m = moment(date);
  return m.format("jYYYY");
};
