...
export const getGregorianDateFromPersian = (year, month) => {
  ...
  const jdateStart = new JDate(persianYear, persianMonth - 1, 1);
  const jdateEnd = new JDate(
    persianYear,
    persianMonth - 1,
    getPersianMonthLength(persianYear, persianMonth),
  );

  const gregorianStart = jdateStart.getGregorianDate();
  const gregorianEnd = jdateEnd.getGregorianDate();

  gregorianEnd.setHours(23, 59, 59, 999);

  return { start: gregorianStart, end: gregorianEnd };
};

const getPersianMonthLength = (year, month) => {
  if (month <= 6) {
    return 31;
  }
  if (month <= 11) {
